...
| `ALLOW_PURGE` | `1` | app — master switch for `POST /admin/purge` (the operator "delete all detections" control); `0` ⇒ 403 and nothing is deleted |
...
| POST | `/admin/purge` | body `{"confirm": "DELETE", "scope": "all"\|"simulated"?}` → `{"deleted": int, "remaining": int}`. Empties the `packets` table so every count reads zero, or with `scope:"simulated"` deletes only rows tagged `raw.sim` (decided in Python, so identical on SQLite and PostgreSQL) and leaves captured frames. The `confirm` sentinel is fixed and **required** — anything but the exact string `DELETE` (an empty body included) is **400** and deletes nothing. **403** when `ALLOW_PURGE=0`. The operator "delete all detections" control on `/admin` calls this; it is deliberately **not** routed through Saqr (whose `delete_detections` refuses an unfiltered wipe). Unauthenticated like the rest of the operator surface — gate it with `ALLOW_PURGE=0` on any deployment where the page is reachable by an untrusted party |
